...
docInput.onchange=()=>{
  ...
  docList.innerHTML=tempDocs.map((d,i)=>`<li>${d}<button onclick="removeDoc(${i})">❌</button></li>`).join("");
};
...
openFormBtn.onclick=()=>{tempDocs=[];docList.innerHTML="";recordForm.reset();recordModal.style.display="block";}
